...
`Codespark` component has an out-of-box internal workspace. You can use `getWorkspace` props to access it.
...
<Callout>To reduce the complexity of the `Codespark` component and maintain out-of-the-box features, please use composable components when you need to create a custom workspace.</Callout>
...
Another way to create a workspace is to use the `useWorkspace` hooks.
