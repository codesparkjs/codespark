...
Code block directives support passing string-type and boolean-type props to the corresponding components, such as `height` and `width`. For more details, see the [API reference](/docs/components/codespark#props) and [Remark Plugin](/docs/plugin/remark) documentation.
