...
`createWorkspace` is a factory function that creates a [Workspace](/docs/functions/workspace) instance from a React component or element.

<Callout title="Plugin Required" type="warn">The `createWorkspace` function performs static code analysis to extract dependencies. You need to install the [required plugin](/docs/plugin-list) before using it.</Callout>
...
<Codespark files={files} />
```

**After** — using `createWorkspace`:
...
With this approach, you import and use components just like you would in any React application.
