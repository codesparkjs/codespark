...
1. The framework accepts `.md` files as input
2. Markdown content is parsed and converted to HTML using `marked`
3. The resulting HTML is sanitized through `DOMPurify` to remove potentially malicious content
4. The sanitized HTML is rendered in the preview panel
