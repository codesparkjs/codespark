...
The plugin registers a custom loader that executes during Webpack's module processing phase and performs the following steps:

### 1. AST Parsing

The loader parses JavaScript/TypeScript files (`.js`, `.jsx`, `.ts`, `.tsx`) using the Babel parser with JSX and TypeScript support.

### 2. Import Detection

The plugin identifies imports from `@codespark/react` that match the configured method names (default: `createWorkspace`).
...
Consider a documentation page with an interactive component preview:
...
2. Analyze `ButtonDemo` and discover its dependency on `Button` from `./components/button`
3. Collect the `Button` component source code
4. Inject the collected information into the `createWorkspace` call

The resulting workspace will contain both the entry code and the `Button` component source, enabling the live preview to render correctly.

## Alias Resolution

The plugin supports TypeScript path aliases defined in `tsconfig.json`:
...
Imports using path aliases will be correctly resolved:
...
- Circular dependencies are handled but may result in incomplete dependency collection
