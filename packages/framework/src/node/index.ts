...
import { CSSLoader, ESLoader, JSONLoader, LoaderType, SVGLoader } from '../loaders';
import type { Outputs } from '../registry';
import { NodeAnalyzer } from './analyze';
...
export class Framework extends Base {
  ...
  analyze(files: Record<string, string>) {
    const analyzer = new NodeAnalyzer(files, [new ESLoader({ transform: true })]);
    analyzer.analyze();
    this.outputs = analyzer.outputs;
  }
...
export class ViteFramework extends NodeFramework<ViteFrameworkEvents> {
  ...
  analyze(files: Record<string, string>) {
    const analyzer = new NodeAnalyzer(files, [new ESLoader({ isTSX: true }), new JSONLoader(), new CSSLoader(), new SVGLoader()]);
    analyzer.analyze();
    this.outputs = analyzer.outputs;
  }
  ...
  setHMRTarget(target: Window) {
    if (!this.devServer) return;

    this.devServer.setHMRTarget(target);
    this.devServer.on('hmr-update', update => {
