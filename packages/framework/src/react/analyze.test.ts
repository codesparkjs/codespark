...
describe('analyze', () => {
  it('should return empty outputs when no imports', () => {
    const outputs = analyze({ [ENTRY]: '<div />' });
  ...
  it('should handle external deps', () => {
    const outputs = analyze({ [ENTRY]: "import { useState } from 'react';\nuseState()" });
    const esModules = outputs.get(LoaderType.ESModule) as Output<LoaderType.ESModule>[];
    expect(esModules).toHaveLength(1);
  ...
  it('should handle URL imports', () => {
    const outputs = analyze({ [ENTRY]: "import confetti from 'https://esm.sh/canvas-confetti@1.6.0';\nconfetti()" });
    const esModules = outputs.get(LoaderType.ESModule) as Output<LoaderType.ESModule>[];
    expect(esModules[0].externals).toHaveLength(1);
  ...
  it('should build internal dep from files map', () => {
    ...
    const entryModule = esModules.find(m => m.path === ENTRY)!;
    expect(entryModule.dependencies['./button.tsx']).toBe('./button.tsx');
  });
  ...
  it('should resolve import without extension', () => {
    ...
    expect(esModules).toHaveLength(2);
    const entryModule = esModules.find(m => m.path === ENTRY)!;
    expect(entryModule.dependencies['./button']).toBe('./button.tsx');
  });
  ...
  it('should resolve import to folder index', () => {
    ...
    expect(esModules).toHaveLength(2);
    const entryModule = esModules.find(m => m.path === ENTRY)!;
    expect(entryModule.dependencies['./button']).toBe('./button/index.tsx');
  });
  ...
  it('should skip type-only imports', () => {
    ...
    const esModules = outputs.get(LoaderType.ESModule) as Output<LoaderType.ESModule>[];
    // analyze now processes all files, type-only imports are handled at compile time
    expect(esModules).toHaveLength(2);
  });
  ...
  it('should skip unused imports', () => {
    ...
    const esModules = outputs.get(LoaderType.ESModule) as Output<LoaderType.ESModule>[];
    // analyze now processes all files, unused imports are handled at compile time
    expect(esModules).toHaveLength(2);
  });
  ...
  it('should handle JSON imports', () => {
    ...
    expect(esModules).toHaveLength(2);
    const entryModule = esModules.find(m => m.path === ENTRY)!;
    expect(entryModule.dependencies['./data.json']).toBe('./data.json');
  });
});
